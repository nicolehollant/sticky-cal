...
* @param {string} path - The path to the ics file to parsed
* @param {DateTime} from - The datetime corresponding to the weel
* @returns {object} - An object containing relevant info
*/
const week = (path, from = DateTime.local()) => {
  // console.log(require('fs').readdirSync('src/assets/'))
  const events = ical.sync.parseFile(path) // events from ics file
  // const from = DateTime.local() // current time
  ...
  // const weekEvents = [] // all events in current week from ics file
  const weekEvents = {
    0: {
